"""
Agent Researcher Service - Investment Advice Agent
"""
...
app = FastAPI(title="Agent Researcher Service")
...
async def run_research_agent(topic: str) -> str:
    """Run the research agent to get related information."""

    # Prepare the user query
    query = f"Please research recent news related to the phenomenon of this topic {topic} across China, Japan, Korea, the UK, the US, Canada, and Australia. Pick something trending or significant happening in these countries right now."

    # Please override these variables with the region you are using
    # Other choices: ap-southeast-2 (for OpenAI OSS models) and eu-central-1
...
async def health():
    ...
    return {
        "service": "Agent Researcher",
        "status": "healthy",
        "agent_api_configured": bool(os.getenv("AGENT_API_ENDPOINT") and os.getenv("AGENT_API_KEY")),
        "timestamp": datetime.now(UTC).isoformat(),
...
async def test_bedrock():
    ...
    try:
        ...
        # Try to create Bedrock client explicitly in ap-southeast-2
        client = boto3.client("bedrock-runtime", region_name="ap-southeast-2")

        # Debug: Try to list models to verify connection
        try:
            bedrock_client = boto3.client("bedrock", region_name="ap-southeast-2")
